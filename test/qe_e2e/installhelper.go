...
func (receiver *installHelper) createClusterAROCommonBuilder() *createCluster {
	...
	return &createCluster{
		...
		AzureCreds:          compat_otp.MustGetAzureCredsLocation(),
		...
		PullSecret:          compat_otp.GetTestEnv().PullSecretLocation,
		ReleaseImage:        compat_otp.GetLatestReleaseImageFromEnv(),
	}
}
...
func (receiver *installHelper) newAWSS3Client() string {
	...
	receiver.s3Client = compat_otp.NewS3ClientFromCredFile(filePath, "default", region)
...
func (receiver *installHelper) dumpHostedCluster(createCluster *createCluster) error {
	...
	compat_otp.MoveFileToPath(path.Join(dumpDir, dumpArchiveName), path.Join(dumpArtifactDir, dumpArchiveName))
