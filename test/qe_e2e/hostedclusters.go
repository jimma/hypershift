...
func (h *hostedCluster) validateClusterAutoScallerDeploymentRunning() {
	// the cluster-autoscaler deployment replicas is >= 1
	o.Eventually(func() int {
		auto_scaler_replicas, err := h.oc.AsAdmin().WithoutNamespace().Run("get").Args("deployment", "cluster-autoscaler", "-n", h.getHostedComponentNamespace(), "-o=jsonpath={.spec.replicas}").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		r, err := strconv.Atoi(auto_scaler_replicas)
		o.Expect(err).NotTo(o.HaveOccurred())
		return r
	}).WithTimeout(5 * time.Minute).WithPolling(10 * time.Second).Should(o.BeNumerically(">=", 1))

	// the cluster-autoscaler pod is running
	o.Eventually(func() bool {
		clusterAutoScalerPods, _, err := h.oc.AsAdmin().WithoutNamespace().Run(OcpGet).
			Args("pod", "-n", h.getHostedComponentNamespace(), "-l", "app=cluster-autoscaler", "-l", "hypershift.openshift.io/control-plane-component=cluster-autoscaler").Outputs()
		return err == nil && len(clusterAutoScalerPods) > 0
	}).WithTimeout(5 * time.Minute).WithPolling(10 * time.Second).Should(o.BeTrue())
}

// getHostedClusterUnschedulableNodeCount get nodes count which has '.spec.unschedulable: true'
// name: npName name, if empty, get all ready nodes' count
func (h *hostedCluster) getHostedClusterUnschedulableNodeCount(npName string) (int, error) {
	return h.getHostedClusterNodeCount(npName, []string{`-ojsonpath='{.items[*].spec.unschedulable}'`}, "true")
}

func (h *hostedCluster) getHostedClusterNodeCount(npName string, cond []string, value string) (int, error) {
	_cond := []string{"--kubeconfig=" + h.hostedClustersKubeconfigFile, "node", "--ignore-not-found"}
	if len(npName) > 0 {
		_cond = append(_cond, "-l", "hypershift.openshift.io/nodePool="+npName)
	}
	if len(cond) > 0 {
		_cond = append(_cond, cond...)
	}
	_value, er := h.oc.AsAdmin().WithoutNamespace().Run("get").Args(_cond...).Output()
	if er != nil {
		e2e.Logf("get node counts error: %v", er)
		return 0, er
	}
	return strings.Count(_value, value), nil
}
...
func (h *hostedCluster) getAzureNodePoolImageType(npName string) azureNodepoolImageType {
	return azureNodepoolImageType(doOcpReq(h.oc, OcpGet, true, "np", npName, "-n", h.namespace, "-o=jsonpath={.spec.platform.azure.image.type}"))
}
...
func (h *hostedCluster) setClusterAutoScale(hcClusterName, autoScalingJson string) {
	clusterAutoScaleConfig := fmt.Sprintf(`--patch={"spec": {"autoscaling": %s}}`, autoScalingJson)
	_, err := h.oc.AsAdmin().WithoutNamespace().Run(OcpPatch).Args("-n", h.namespace, "hostedcluster", hcClusterName, clusterAutoScaleConfig, "--type=merge").Output()
	o.Expect(err).ShouldNot(o.HaveOccurred())
}

func (h *hostedCluster) disableClusterAutoScale(hcClusterName string) {
	removeAuoScalerConfig := `[{"op": "remove", "path": "/spec/autoscaling"}]`
	_, err := h.oc.AsAdmin().WithoutNamespace().Run(OcpPatch).Args("-n", h.namespace, "hostedcluster", hcClusterName, "--type=json", "-p", removeAuoScalerConfig).Output()
	o.Expect(err).ShouldNot(o.HaveOccurred())
}

func (h *hostedCluster) disableNodePoolAutoScale(nodePoolName string) {
	removeNodePoolAuoScalerConfig := `[{"op": "remove", "path": "/spec/autoScaling"}]`
	_, err := h.oc.AsAdmin().WithoutNamespace().Run(OcpPatch).Args("-n", h.namespace, "nodepool", nodePoolName, "--type=json", "-p", removeNodePoolAuoScalerConfig).Output()
	o.Expect(err).ShouldNot(o.HaveOccurred())
}
...
// getAWSInstanceIDs retrieves the AWS EC2 instance IDs associated with the given node pool name.
// It returns a slice of instance ID strings ordered by the creationTime(Age). The first item is the
// oldest one.
// nodepoolName - the name of the node pool to query for associated AWS instances.
func (h *hostedCluster) getAWSInstanceIDs(nodepoolName string) []string {
	h.checkAWSNodepoolInstanceType(nodepoolName, "AWS")
	awsFullInstanceIDs, awsInstanceErr := h.oc.AsGuestKubeconf().AsAdmin().
		WithoutNamespace().Run(OcpGet).Args("nodes", "--sort-by={metadata.creationTimestamp}",
		"-l hypershift.openshift.io/nodePool="+nodepoolName,
		"-ojsonpath={.items[*].spec.providerID}").Output()
	o.Expect(awsInstanceErr).NotTo(o.HaveOccurred())
	return strings.Split(awsFullInstanceIDs, " ")
}
...
func (h *hostedCluster) getCPEtcdLeaderAndFollowers() (string, []string, error) {
	...
	for i := 0; i < totalNum; i++ {
		...
		res, err := compat_otp.RemoteShPodWithBashSpecifyContainer(h.oc, h.namespace+"-"+h.name, podName, "etcd", etcdEndpointStatusCmd)
		o.Expect(err).ShouldNot(o.HaveOccurred())
...
func (h *hostedCluster) isCPEtcdPodHealthy(podName string) bool {
	...
	res, err := compat_otp.RemoteShPodWithBashSpecifyContainer(h.oc, h.namespace+"-"+h.name, podName, "etcd", etcdEndpointHealthCmd)
	if err != nil {
...
func (h *hostedCluster) checkSecretEncryptionDecryption(isEtcdEncrypted bool) {
	var (
		secretName  = fmt.Sprintf("etcd-encryption-%s", strings.ToLower(compat_otp.RandStrDefault()))
		secretNs    = "default"
		secretKey   = "foo"
		secretValue = "bar"
	)
	...
	encryptedSecretContent, err := compat_otp.RemoteShPodWithBashSpecifyContainer(h.oc, h.getHostedComponentNamespace(),
		"etcd-0", "etcd", etcdCmd)
...
func (h *hostedCluster) waitForKASDeployUpdate(ctx context.Context, oldResourceVersion string) {
	...
	err := compat_otp.WaitForResourceUpdate(ctx, h.oc, DefaultTimeout/20, DefaultTimeout,
...
func (h *hostedCluster) waitForKASDeployReady(ctx context.Context) {
	...
	compat_otp.WaitForDeploymentsReady(ctx, func(ctx context.Context) (*appsv1.DeploymentList, error) {
	...
	}, compat_otp.IsDeploymentReady, LongTimeout, LongTimeout/20, true)
}
...
func (h *hostedCluster) patchNodeLabelToNodePool(nodePool, nodeLabelPatch string) {
	nodePoolNodeLabelPatch := fmt.Sprintf(`--patch={"spec": {"nodeLabels": %s}}`, nodeLabelPatch)
	_, err := h.oc.AsAdmin().WithoutNamespace().Run(OcpPatch).Args("-n", h.namespace, "nodepool", nodePool, nodePoolNodeLabelPatch, "--type=merge").Output()
	o.Expect(err).ShouldNot(o.HaveOccurred())
}

func (h *hostedCluster) controlPlaneNamespace() string {
