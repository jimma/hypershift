...
var _ = g.Describe("[sig-hypershift] Hypershift", func() {
	...
	g.It("Author:heli-HyperShiftMGMT-Longduration-NonPreRelease-Critical-43272-Critical-43829-Test cluster autoscaler via hostedCluster autoScaling settings [Serial]", func() {
		...
		compat_otp.By("create a nodepool")
		defer func() {
		...
		compat_otp.By("enable the nodepool to be autoscaling")
		...
		compat_otp.By("create a job as workload in the hosted cluster")
		wl := workload{
			name:      "workload",
			namespace: "default",
		...
		compat_otp.By("check nodepool is auto-scaled to max")
	...
	g.It("HyperShiftMGMT-ROSA-Author:heli-Critical-45770-Test basic fault resilient HA-capable etcd[Serial][Disruptive]", func() {
		...
		for i := 0; i < 3; i++ {
			...
			if strings.Contains(res, "false, false") {
				...
				compat_otp.AssertWaitPollNoErr(err, "etcd cluster health check error")
	...
	g.It("Author:heli-HyperShiftMGMT-ROSA-Critical-45801-Critical-45821-Test fault resilient HA-capable etcd under network partition[Disruptive]", func(ctx context.Context) {
		...
		compat_otp.SkipOnAKSNess(ctx, oc, false)
		...
		defer func() {
			debugNodeStdout, err := compat_otp.DebugNodeWithChroot(oc, etcdNodeMap[followers[0]], "iptables", "-t", "filter", "-D", "INPUT", "-s", etcdNodeMap[leader], "-j", "DROP")
			o.Expect(err).ShouldNot(o.HaveOccurred())
		...
		debugNodeStdout, err := compat_otp.DebugNodeWithChroot(oc, etcdNodeMap[followers[0]], "iptables", "-t", "filter", "-A", "INPUT", "-s", etcdNodeMap[leader], "-j", "DROP")
		o.Expect(err).ShouldNot(o.HaveOccurred())
		e2e.Logf("drop traffic debug output 1: %s", debugNodeStdout)

		g.By("drop traffic from follower to leader")
		defer func() {
			debugNodeStdout, err := compat_otp.DebugNodeWithChroot(oc, etcdNodeMap[leader], "iptables", "-t", "filter", "-D", "INPUT", "-s", etcdNodeMap[followers[0]], "-j", "DROP")
			o.Expect(err).ShouldNot(o.HaveOccurred())
		...
		debugNodeStdout, err = compat_otp.DebugNodeWithChroot(oc, etcdNodeMap[leader], "iptables", "-t", "filter", "-A", "INPUT", "-s", etcdNodeMap[followers[0]], "-j", "DROP")
		o.Expect(err).ShouldNot(o.HaveOccurred())
	...
	g.It("HyperShiftMGMT-Author:heli-Critical-44824-Resource requests/limit configuration for critical control plane workloads[Serial][Disruptive]", func() {
		...
		defer func() {
			...
			compat_otp.AssertWaitPollNoErr(err, "kube-apiserver cpu & memory resource change back error")
		}()
		...
		compat_otp.AssertWaitPollNoErr(err, "kube-apiserver cpu & memory resource update error")
	})
	...
	g.It("HyperShiftMGMT-Author:heli-NonPreRelease-Longduration-Critical-44942-Enable control plane deployment restart on demand[Serial]", func() {
		...
		compat_otp.AssertWaitPollNoErr(err, "ocp-44942 hostedcluster restart annotation not found error")
	})
	...
	// author: heli@redhat.com
	g.It("Author:heli-HyperShiftMGMT-Longduration-NonPreRelease-Critical-48025-Test EBS allocation for nodepool[Disruptive]", func() {
		if iaasPlatform != "aws" {
		...
		// create nodepools without waiting to speed up the test
		for _, cf := range awsConfigs {
	...
	g.It("HyperShiftMGMT-Longduration-NonPreRelease-Author:heli-Critical-43553-Test MHC through nodePools[Disruptive]", func() {
		...
		npName := "43553np-" + strings.ToLower(compat_otp.RandStrDefault())
		replica := 2
		releaseImage := doOcpReq(oc, OcpGet, true, "hostedcluster", hostedcluster.name, "-n", hostedcluster.namespace, "-ojsonpath={.spec.release.image}")
		...
		nsName := "guest-43553" + strings.ToLower(compat_otp.RandStrDefault())
	...
	g.It("HyperShiftMGMT-NonPreRelease-Longduration-Author:mihuang-Critical-48673-Unblock node deletion-draining timeout[Serial]", func() {
		...
		npName := "48673np-" + strings.ToLower(compat_otp.RandStrDefault())
		replica := 1
		releaseImage := doOcpReq(oc, OcpGet, true, "hostedcluster", hostedcluster.name, "-n", hostedcluster.namespace, "-ojsonpath={.spec.release.image}")
		defer func() {
			hostedcluster.deleteNodePool(npName)
	...
	g.It("HyperShiftMGMT-NonPreRelease-Longduration-Author:mihuang-Critical-49436-Test Nodepool conditions[Serial]", func() {
		...
		replica := 1
		npNameInPlace := "49436np-inplace-" + strings.ToLower(compat_otp.RandStrDefault())
		npNameReplace := "49436np-replace-" + strings.ToLower(compat_otp.RandStrDefault())
		...
		failOut, err := oc.AsAdmin().WithoutNamespace().Run(OcpPatch).Args("nodepools", npNameReplace, "-n", hostedcluster.namespace, "--type=merge", fmt.Sprintf(`--patch={"spec": {"replicas": 2}}`)).Output()
		o.Expect(err).Should(o.HaveOccurred())
		o.Expect(failOut).Should(o.ContainSubstring("Both replicas or autoScaling should not be set"))
		...
		npNameMinVersion := "49436np-minversion-" + strings.ToLower(compat_otp.RandStrDefault())
		minSupportedVersion := getMinSupportedOCPVersion()
		minReleaseImage := fmt.Sprintf(`quay.io/openshift-release-dev/ocp-release:%s-x86_64`, minSupportedVersion)
		defer hostedcluster.deleteNodePool(npNameMinVersion)
		NewAWSNodePool(npNameMinVersion, hostedcluster.name, hostedcluster.namespace).WithNodeCount(&replica).WithReleaseImage(minReleaseImage).CreateAWSNodePool()
		o.Eventually(hostedcluster.pollCheckNodePoolConditions(npNameMinVersion, []nodePoolCondition{{"ValidReleaseImage", "message", minSupportedVersion}}), LongTimeout, LongTimeout/10).Should(o.BeTrue(), "nodepool in progress error")
	})
	...
	g.It("HyperShiftMGMT-Author:liangli-Critical-54284-Hypershift creates extra EC2 instances", func() {
		...
		o.Expect(len(infras)).Should(o.BeEquivalentTo(1))
		infra := infras[0]
		awsMachineTemplateStr := doOcpReq(oc, OcpGet, true, "awsmachinetemplate", "-n", hostedcluster.namespace+"-"+hostedcluster.name, `-ojsonpath={.items[*].metadata.name}`)
		awsMachineTemplates := strings.Split(awsMachineTemplateStr, " ")
		for _, awsMachineTempate := range awsMachineTemplates {
			additionalTags := doOcpReq(oc, OcpGet, true, "awsmachinetemplate", "-n", hostedcluster.namespace+"-"+hostedcluster.name, awsMachineTempate, fmt.Sprintf(`-ojsonpath={.spec.template.spec.additionalTags.kubernetes\.io/cluster/%s}`, infra))
	...
	g.It("HyperShiftMGMT-Longduration-NonPreRelease-Author:liangli-Critical-54551-Reconcile NodePool label against Nodes[Disruptive]", func() {
		...
		replica := 1
		nodepoolName := "54551np-" + strings.ToLower(compat_otp.RandStrDefault())
	...
	g.It("Author:mihuang-ROSA-OSD_CCS-HyperShiftMGMT-Longduration-NonPreRelease-Critical-49108-Critical-49499-Critical-59546-Critical-60490-Critical-61970-Separate client certificate trust from the global hypershift CA", func(ctx context.Context) {
		...
		hcNS := hostedcluster.namespace + "-" + hostedcluster.name
		o.Eventually(func() bool {
			secretName := "prometheus-k8s-tls-assets-0"
			keyStr := doOcpReq(oc, OcpGet, true, "secret", secretName, "-n", "openshift-monitoring", "-o=go-template='{{range $k, $v := .data}}{{printf \"%s \" $k}}{{end}}'")
			e2e.Logf("keys in secret: %s are: %s", secretName, keyStr)
			keys := strings.Split(keyStr, " ")
			rootCA := FindBySuffix(keys, fmt.Sprintf("%s_root-ca_ca.crt", hcNS))
			metricsClientKey := FindBySuffix(keys, fmt.Sprintf("%s_metrics-client_tls.key", hcNS))
			metricsClientCert := FindBySuffix(keys, fmt.Sprintf("%s_metrics-client_tls.crt", hcNS))
			if rootCA == "" || metricsClientKey == "" || metricsClientCert == "" {
				e2e.Logf("Required tls assets not found yet.")
				return false
			}

			metricsOutput, err := oc.AsAdmin().Run("exec").Args("-n", "openshift-monitoring", "prometheus-k8s-0", "-c", "prometheus", "--", "sh", "-c", fmt.Sprintf("curl -sS --cacert /etc/prometheus/certs/%s --key /etc/prometheus/certs/%s --cert /etc/prometheus/certs/%s https://openshift-apiserver.%s.svc/metrics", rootCA, metricsClientKey, metricsClientCert, hcNS)).Output()
			if err != nil {
				return false
			}
			// The expected metrics doesn't matter, we just want to make sure the metrics can be scraped successfully.
			expectedMetric1 := "# HELP aggregator_openapi_v2_regeneration_count [ALPHA] Counter of OpenAPI v2 spec regeneration count broken down by causing APIService name and reason."
			expectedMetric2 := "# HELP aggregator_discovery_aggregation_count_total [ALPHA] Counter of number of times discovery was aggregated"
			return strings.Contains(metricsOutput, expectedMetric1) || strings.Contains(metricsOutput, expectedMetric2)
		}, 5*LongTimeout, LongTimeout/5).Should(o.BeTrue(), fmt.Sprintf("not all metrics in hostedcluster %s are ready", hostedcluster.name))
	...
	g.It("HyperShiftMGMT-Longduration-NonPreRelease-Author:mihuang-Critical-60744-Better signal for NodePool inability to talk to management side [Disruptive] [Flaky]", func() {
		...
		replica := 1
		nodepoolName := "60744np-" + strings.ToLower(compat_otp.RandStrDefault())
		defer hostedcluster.deleteNodePool(nodepoolName)
		hostedcluster.createAwsNodePool(nodepoolName, replica)
		o.Eventually(hostedcluster.pollCheckHostedClustersNodePoolReady(nodepoolName), DoubleLongTimeout, DoubleLongTimeout/10).Should(o.BeTrue(), "nodepool ready error")
		...
		nodepoolName1 := "60744np-1-" + strings.ToLower(compat_otp.RandStrDefault())
	...
	g.It("HyperShiftMGMT-Author:mihuang-Critical-61604-Validate network input and signal in hyperv1.ValidHostedClusterConfiguration[Disruptive]", func() {
		g.By("Patch hostedcluster to set clusterNetwork which should fail and the clusterNetwork cidr does not change.")
		oldClusterNetworkCidr := doOcpReq(oc, OcpGet, true, "hostedcluster", hostedcluster.name, "-n", hostedcluster.namespace, "-o", `jsonpath={.spec.networking.clusterNetwork[0].cidr}`)
		_, err := oc.AsAdmin().WithoutNamespace().Run("patch").Args("hostedcluster", hostedcluster.name, "-n", hostedcluster.namespace, "--type", "merge", "-p", `{"spec":{"networking":{"clusterNetwork":[{"cidr": "172.31.0.0/16"}]}}}`).Output()
		// the error is expected becaues the validation is done at API spec level.
		o.Expect(err).Should(o.HaveOccurred())
		...
		// it just does not take any effect
		o.Expect(clusterNetworkCidr).To(o.Equal(oldClusterNetworkCidr))
	...
	g.It("HyperShiftMGMT-Author:mihuang-Critical-62195-Add validation for taint.value in nodePool[Serial][Disruptive]", func() {
		...
		nodepoolName := "62195np" + strings.ToLower(compat_otp.RandStrDefault())
		defer func() {
	...
	g.It("HyperShiftMGMT-Longduration-NonPreRelease-Author:heli-Critical-60140-[AWS]-create default security group when no security group is specified in a nodepool[Serial]", func() {
		...
		groupID, err := awsClient.CreateSecurityGroup(fmt.Sprintf("ocp-60140-sg-%s", strings.ToLower(compat_otp.RandStrDefault())), vpcID, "hypershift ocp-60140")
		o.Expect(err).ShouldNot(o.HaveOccurred())
...
`
	...
	g.It("HyperShiftMGMT-Author:liangli-Critical-48511-Test project configuration resources on the guest cluster[Serial]", func() {
		...
		o.Eventually(func() error {
			...
			return err
		}).WithTimeout(DefaultTimeout).WithPolling(DefaultTimeout / 10).ShouldNot(o.HaveOccurred())
	...
	g.It("HyperShiftMGMT-NonPreRelease-Longduration-Author:fxie-Critical-67786-Changes to NodePool .spec.platform should trigger a rolling upgrade [Serial]", func() {
		...
		compat_otp.By("Creating an additional NodePool")
		...
		compat_otp.By("Checking instance type on CAPI resources")
		...
		compat_otp.By("Checking instance type label on nodes belonging to the newly created NodePool")
		...
		compat_otp.By(fmt.Sprintf("Change instance type to %s", npInstanceTypeNew))
		...
		compat_otp.By("Waiting for replace upgrade to complete")
		upgradeType := hostedcluster.getNodepoolUpgradeType(npName)
		o.Expect(upgradeType).Should(o.ContainSubstring("Replace"))
		...
		compat_otp.By("Make sure the instance type is updated on CAPI resources")
		awsMachineTemp, err = hostedcluster.getCurrentInfraMachineTemplatesByNodepool(context.Background(), npName)
		o.Expect(err).ShouldNot(o.HaveOccurred())
		instanceType, found, err = unstructured.NestedString(awsMachineTemp.Object, "spec", "template", "spec", "instanceType")
		o.Expect(found).To(o.BeTrue())
		...
		compat_otp.By("Make sure the node instance types are updated as well")
		nodeList, err = oc.GuestKubeClient().CoreV1().Nodes().List(context.Background(), metav1.ListOptions{
			LabelSelector: labels.SelectorFromSet(map[string]string{
				hypershiftNodePoolLabelKey: npName,
...
`, cmNamePrefix, cmIdx, cmNs, cmData)
		...
		compat_otp.By("Making sure the (hosted) control plane is highly available by checking the number of etcd Pods")
		...
		compat_otp.By("Getting DB size of an ETCD member")
		...
		compat_otp.By("Creating ConfigMaps on the guest cluster until the ETCD member DB size is large enough")
		...
		compat_otp.By("Deleting all ConfigMaps")
		_, err = oc.AsGuestKubeconf().Run("delete").Args("cm", "-n=default", "-l=foo=bar").Output()
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("Waiting until the fragmentation ratio is above threshold+margin")
		o.Eventually(func() (done bool) {
		...
		compat_otp.By("Waiting until defragmentation is done which causes DB size to decrease")
		o.Eventually(func() (done bool) {
		...
		_, err = compat_otp.WaitAndGetSpecificPodLogs(oc, hcpNs, "etcd", "etcd-0", "defrag")
		o.Expect(err).NotTo(o.HaveOccurred())
	})
	...
	*/
	g.It("HyperShiftMGMT-NonPreRelease-Longduration-Author:fxie-Critical-72055-Automated etcd backups for Managed services", func() {
		...
		compat_otp.By("Getting an AWS session with credentials obtained from cluster profile")
		...
		compat_otp.By("Getting AWS account ID")
		stsClient := compat_otp.NewDelegatingStsClient(sts.New(sess))
		...
		compat_otp.By("Getting SA issuer of the management cluster")
		...
		compat_otp.By("Creating AWS role")
		iamClient := compat_otp.NewDelegatingIAMClient(awsiam.New(sess))
		...
		compat_otp.By("Creating AWS S3 bucket")
		s3Client := compat_otp.NewDelegatingS3Client(s3.New(sess))
		...
		compat_otp.By("Creating CM/etcd-backup-config")
		...
		compat_otp.By("Waiting for the etcd backup CronJob to be created")
		o.Eventually(func() bool {
		...
		compat_otp.By("Waiting for the first job execution to be successful")
		o.Eventually(func() bool {
		...
		compat_otp.By("Waiting for the backup to be uploaded")
		o.Expect(s3Client.WaitForBucketEmptinessWithContext(ctx, etcdBackupBucketName,
			compat_otp.BucketNonEmpty, 5*time.Second /* Interval */, 1*time.Minute /* Timeout */)).NotTo(o.HaveOccurred())
	})

	g.It("Author:xiuwang-ARO-HyperShiftMGMT-Critical-83028-Check hosted_cluster_managed_azure_info metric", func(ctx context.Context) {
		isAKS, _ := compat_otp.IsAKSCluster(ctx, oc)
		if !isAKS {
			g.Skip("Skip the test as it is only for ARO HCP cluster")
		}
		envList := doOcpReq(oc, OcpSet, true, "env", "deploy/operator", "-n", "hypershift", "--list")
		var hostedClusterAzureMetricsName string
		if strings.Contains(envList, managedServiceKey+"="+managedServiceAROHCP) {
			hostedClusterAzureMetricsName = "hosted_cluster_managed_azure_info"
		} else {
			hostedClusterAzureMetricsName = "hosted_cluster_azure_info"
		}
		hypershiftOperatorPodName := doOcpReq(oc, OcpGet, true, "pod", "-n", "hypershift", "-l", "app=operator", `-ojsonpath={.items[0].metadata.name}`)
		metrics := doOcpReq(oc, OcpExec, true, "-n", "hypershift", hypershiftOperatorPodName, "--", "curl", "0.0.0.0:9000/metrics")
		o.Expect(metrics).To(o.ContainSubstring(hostedClusterAzureMetricsName))
	})
