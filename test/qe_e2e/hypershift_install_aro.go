...
var _ = g.Describe("[sig-hypershift] Hypershift [HyperShiftAKSINSTALL]", func() {
	...
	g.BeforeEach(func(ctx g.SpecContext) {
		compat_otp.SkipOnAKSNess(ctx, oc, true)
		compat_otp.SkipOnHypershiftOperatorExistence(oc, true)
	...
	g.It("Author:fxie-Longduration-NonPreRelease-Critical-74561-Test basic proc for shared ingress [Serial]", func(ctx context.Context) {
		var (
			...
			artifactDir        = path.Join(compat_otp.GetTestEnv().ArtifactDir, "hypershift-artifact", resourceNamePrefix)
			installhelper      = installHelper{oc: oc, dir: tempDir, artifactDir: artifactDir}
		)

		createTempDir(tempDir)

		compat_otp.By("Creating two HCs simultaneously")
		...
		compat_otp.By("Making sure that a shared ingress is used by both HCs")
		...
		compat_otp.By("Scaling up the existing NodePools")
		...
		compat_otp.By("Ensuring that the shared ingress properly manages concurrent traffic coming from external DNS")
	...
	g.It("Author:fxie-Longduration-NonPreRelease-Critical-49173-Critical-49174-Test AZURE node root disk size [Serial]", func(ctx context.Context) {
		var (
			...
			artifactDir        = path.Join(compat_otp.GetTestEnv().ArtifactDir, "hypershift-artifact", resourceNamePrefix)
			installhelper      = installHelper{oc: oc, dir: tempDir, artifactDir: artifactDir}
			azClientSet        = compat_otp.NewAzureClientSetWithCredsFromCanonicalFile()
		)

		createTempDir(tempDir)

		compat_otp.By("Creating HostedCluster")
		...
		compat_otp.By("Creating additional NodePool")
		...
		compat_otp.By("Waiting for the HC and the NP to be ready")
		hc.pollUntilReady()
		...
		compat_otp.By("Checking VM root disk size")
		listVMPager := azClientSet.GetVirtualMachinesClient(nil).NewListPager(rgName, &armcompute.VirtualMachinesClientListOptions{})
		err = compat_otp.ProcessAzurePages(ctx, listVMPager, func(page armcompute.VirtualMachinesClientListResponse) error {
	...
	*/
	g.It("Author:fxie-Longduration-NonPreRelease-Critical-73944-AZURE Etcd Encryption [Serial]", func(ctx context.Context) {
		...
		azCreds := compat_otp.NewEmptyAzureCredentialsFromFile()
		err = azCreds.LoadFromFile(compat_otp.MustGetAzureCredsLocation())
	...
	g.It("Author:fxie-Longduration-NonPreRelease-Critical-75856-Create AZURE Infrastructure and Hosted Cluster Separately [Serial]", func() {
		...
		compat_otp.By("Looking up RHCOS image URL for Azure Disk")
		rhcosImage, err := compat_otp.GetRHCOSImageURLForAzureDisk(oc, compat_otp.GetLatestReleaseImageFromEnv(), compat_otp.GetTestEnv().PullSecretLocation, compat_otp.CoreOSBootImageArchX86_64)
		o.Expect(err).NotTo(o.HaveOccurred(), "error getting RHCOS image for Azure Disk")

		compat_otp.By("Creating Infrastructure")
