...
var _ = g.Describe("[sig-hypershift] Hypershift", func() {
	...
	g.BeforeEach(func() {
		...
		fixturePath = compat_otp.FixturePath("testdata", "hypershift")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-42718-[HyperShiftINSTALL] Create a hosted cluster on aws using hypershift tool [Serial]", func() {
		...
		compat_otp.By("Config AWS Bucket And install HyperShift operator")
		installHelper := installHelper{oc: oc, bucketName: "hypershift-" + caseID + "-" + strings.ToLower(compat_otp.RandStrDefault()), dir: dir, iaasPlatform: iaasPlatform}
		defer installHelper.deleteAWSS3Bucket()
		defer installHelper.hyperShiftUninstall()
		installHelper.hyperShiftInstall()

		compat_otp.By("create HostedClusters")
		...
		compat_otp.By("create HostedClusters node ready")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-42866-[HyperShiftINSTALL] Create HostedCluster infrastructure on AWS by using Hypershift CLI [Serial]", func() {
		...
		compat_otp.By("create aws HostedClusters")
		createCluster := installHelper.createClusterAWSCommonBuilder().
		...
		compat_otp.By("check vpc is as expected")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-42867-[HyperShiftINSTALL] Create iam and infrastructure repeatedly with the same infra-id on aws [Serial]", func() {
		...
		compat_otp.By("Create the AWS infrastructure 1")
		infraFile := installHelper.dir + "/" + clusterName + "-infra.json"
		infra := installHelper.createInfraCommonBuilder().
		...
		defer installHelper.destroyAWSInfra(infra)
		installHelper.createAWSInfra(infra)
		compat_otp.By("Create AWS IAM resources 1")
		iamFile := installHelper.dir + "/" + clusterName + "-iam.json"
		iam := installHelper.createIamCommonBuilder(infraFile).
			withInfraID(infra.InfraID).
			withOutputFile(iamFile)
		...
		compat_otp.By("Create the AWS infrastructure 2")
		...
		compat_otp.By("Create AWS IAM resources 2")
	...
	g.It("NonPreRelease-Longduration-Author:liangli-Critical-42952-[HyperShiftINSTALL] create multiple clusters without manifest crash and delete them asynchronously [Serial]", func() {
		...
		compat_otp.By("create aws HostedClusters 1")
		...
		compat_otp.By("create aws HostedClusters 2")
		...
		compat_otp.By("delete HostedClusters CR background")
		installHelper.deleteHostedClustersCRAllBackground()
		compat_otp.By("check delete AWS HostedClusters asynchronously")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-44924-[HyperShiftINSTALL] Test multi-zonal control plane components spread with HA mode enabled [Serial]", func() {
		...
		hostedCluster := installHelper.createAWSHostedClustersRender(createCluster, func(filename string) error {
			compat_otp.By("Set HighlyAvailable mode")
			return replaceInFile(filename, "SingleReplica", "HighlyAvailable")
		})

		compat_otp.By("Check if pods of multi-zonal control plane components spread across multi-zone")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-44981-[HyperShiftINSTALL] Test built-in control plane pod tolerations [Serial] [Disruptive]", func() {
		...
		nodes, err := compat_otp.GetClusterNodesBy(oc, "worker")
		o.Expect(err).NotTo(o.HaveOccurred())
		if len(nodes) < 2 {
		...
		compat_otp.By("update taint and label, taint and label use key 'hypershift.openshift.io/cluster'")
		...
		compat_otp.By("Check if control plane pods in HostedClusters are on " + nodes[0])
		...
		compat_otp.By("update taint and label, taint and label use key 'hypershift.openshift.io/control-plane'")
		...
		compat_otp.By("create HostedClusters 2")
		...
		compat_otp.By("Check if control plane pods in HostedClusters are on " + nodes[1])
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-45341-[HyperShiftINSTALL] Test NodePort Publishing Strategy [Serial] [Disruptive]", func() {
		...
		compat_otp.By("Create a nodeport ip bastion")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-47053-[HyperShiftINSTALL] Test InfrastructureTopology configuration [Serial]", func() {
		...
		compat_otp.By("create HostedClusters-1")
		createCluster1 := installHelper.createClusterAWSCommonBuilder().
		...
		compat_otp.By("check HostedClusters-1 HostedClusterInfrastructureTopology")
		...
		compat_otp.By("create HostedClusters-2 infra-availability-policy: HighlyAvailable")
		createCluster2 := installHelper.createClusterAWSCommonBuilder().
		...
		compat_otp.By("check HostedClusters-2 HostedClusterInfrastructureTopology")
		...
		compat_otp.By("Check if pods of multi-zonal components spread across multi-zone")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-48133-[HyperShiftINSTALL] Apply user defined tags to all AWS resources [Serial]", func() {
		...
		compat_otp.By("Confirm user defined tags")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-48672-[HyperShiftINSTALL] Create multi-zone AWS infrastructure and NodePools via CLI [Serial]", func() {
		...
		compat_otp.By("create HostedClusters")
		release, err := compat_otp.GetReleaseImage(oc)
		o.Expect(err).NotTo(o.HaveOccurred())
		createCluster := installHelper.createClusterAWSCommonBuilder().
			withName(clusterName).
			withNodePoolReplicas(1).
		...
		compat_otp.By("Check the hostedcluster and nodepool")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-49129-[HyperShiftINSTALL] Create multi-zone Azure infrastructure and nodepools via CLI [Serial]", func() {
		...
		compat_otp.By("install HyperShift operator")
		defer installHelper.hyperShiftUninstall()
		installHelper.hyperShiftInstall()
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-49173-[HyperShiftINSTALL] Test Azure node root disk size [Serial]", func() {
		...
		compat_otp.By("Check the disk size for the nodepool '" + hostedCluster.name + "'")
		o.Expect(hostedCluster.getAzureDiskSizeGBByNodePool(hostedCluster.name)).Should(o.ContainSubstring("64"))

		compat_otp.By("Get subnet ID of the hosted cluster")
		subnetId := hostedCluster.getAzureSubnetId()
		e2e.Logf("Found subnet ID = %s", subnetId)

		compat_otp.By("create nodepool and check root-disk-size (default 120)")
		...
		compat_otp.By("create nodepool and check root-disk-size (256)")
	...
	g.It("Longduration-NonPreRelease-Author:liangli-Critical-49174-[HyperShiftINSTALL] Create Azure infrastructure and nodepools via CLI [Serial]", func() {
		...
		compat_otp.By("Scale up nodepool")
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		compat_otp.By("Checking tags on the Azure resource group")
		rgName, err := hostedCluster.getResourceGroupName()
		o.Expect(err).NotTo(o.HaveOccurred())
		e2e.Logf("Found resource group name = %s", rgName)
		azClientSet := compat_otp.NewAzureClientSetWithRootCreds(oc)
	...
	g.It("Longduration-NonPreRelease-Author:heli-Critical-62085-Critical-60483-Critical-64808-[HyperShiftINSTALL] The cluster should be deleted successfully when there is no identity provider [Serial]", func() {
		...
		compat_otp.By("Config AWS Bucket And install HyperShift operator")
		bucketName := "hypershift-" + caseID + "-" + strings.ToLower(compat_otp.RandStrDefault())
		region, err := getClusterRegion(oc)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		{
			compat_otp.By("Make sure the API server is exposed via Route")
			o.Expect(hostedCluster.getSvcPublishingStrategyType(hcServiceAPIServer)).To(o.Equal(hcServiceTypeRoute))

			compat_otp.By("Make sure the hosted cluster reports correct control plane endpoint port")
			o.Expect(hostedCluster.getControlPlaneEndpointPort()).To(o.Equal("443"))
		}

		compat_otp.By("delete OpenID connect from aws IAM Identity providers")
		...
		iamClient := compat_otp.NewIAMClient()
		o.Expect(iamClient.DeleteOpenIDConnectProviderByProviderName(provider)).ShouldNot(o.HaveOccurred())

		compat_otp.By("update control plane policy to remove security operations")
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:route53:::hostedzone/Z08584472H531BKOV71X7"
		...
		compat_otp.By("ocp-64808 check hosted condition ValidAWSIdentityProvider should be unknown")
		o.Eventually(func() string {
	...
	g.It("Longduration-NonPreRelease-Author:heli-Critical-60484-[HyperShiftINSTALL] HostedCluster deletion shouldn't hang when OIDC provider/STS is configured incorrectly [Serial]", func() {
		...
		compat_otp.By("Config AWS Bucket")
		bucketName := "hypershift-" + caseID + "-" + strings.ToLower(compat_otp.RandStrDefault())
		region, err := getClusterRegion(oc)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		compat_otp.By("install HO without s3 credentials")
		...
		compat_otp.By("check hosted cluster condition ValidOIDCConfiguration")
		o.Eventually(func() string {
	...
	g.It("Longduration-NonPreRelease-Author:heli-Critical-67828-[HyperShiftINSTALL] non-serving components land on non-serving nodes versus default workers [Serial]", func() {
		...
		if !compat_otp.IsInfrastructuresHighlyAvailable(oc) {
		...
		compat_otp.By("config mgmt cluster: scale a machineseet to repicas==2")
		...
		compat_otp.By("add label/taint to servingComponentNodes")
		defer func() {
		...
		compat_otp.By("add label/taint to nonServingComponentNodes")
		defer func() {
		...
		compat_otp.By("create MachineHealthCheck for serving component machinesets")
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		mhcBaseDir := compat_otp.FixturePath("testdata", "hypershift")
		...
		compat_otp.By("create a hosted cluster")
		release, er := compat_otp.GetReleaseImage(oc)
		o.Expect(er).NotTo(o.HaveOccurred())
		createCluster := installHelper.createClusterAWSCommonBuilder().
			withName("hypershift-"+caseID+"-"+strings.ToLower(compat_otp.RandStr(5))).
			withNodePoolReplicas(2).
		...
		defer func() {
			compat_otp.By("in defer function, destroy the hosted cluster")
			installHelper.destroyAWSHostedClusters(createCluster)

			compat_otp.By("check the previous serving nodes are deleted and new serving nodes are created (machinesets are still in ready status)")
			o.Eventually(func() bool {
				for _, no := range servingComponentNodes {
			...
			compat_otp.By("no cluster label annotation in the new serving nodes")
			for i := 0; i < 2; i++ {
		...
		compat_otp.By("check hostedcluster annotation")
		...
		compat_otp.By("check hosted cluster hcp serving components' node allocation")
		...
		compat_otp.By("check serving nodes hcp labels and taints are generated automatically on the serving nodes")
	...
	g.It("Longduration-NonPreRelease-Author:heli-Critical-67721-[HyperShiftINSTALL] Hypershift Operator version validation is not skipping version checks for node pools [Serial]", func() {
		...
		compat_otp.By("get max unsupported HostedClusters version nightly release")
		maxUnsupportedVersion, err := getVersionWithMajorAndMinor(getLatestUnsupportedOCPVersion())
		o.Expect(err).ShouldNot(o.HaveOccurred())
		release, err := compat_otp.GetLatestNightlyImage(maxUnsupportedVersion)
		o.Expect(err).ShouldNot(o.HaveOccurred())

		compat_otp.By("create HostedClusters with unsupported version")
		createCluster := installHelper.createClusterAWSCommonBuilder().
			withName("hypershift-" + caseID).
			withReleaseImage(release).
			withNodePoolReplicas(1)
		...
		compat_otp.By("check hc condition & nodepool condition")
		o.Eventually(func() bool {
		...
		compat_otp.By("add annotation to skip release check")
		...
		compat_otp.By("check nodepool and hc to be recovered")
		o.Eventually(func() bool {
			hcStatus := doOcpReq(oc, OcpGet, false, "hostedcluster", hc.name, "-n", hc.namespace, "--ignore-not-found", `-o=jsonpath={.status.conditions[?(@.type=="ValidReleaseImage")].status}`)
			if hcStatus != "True" {
				return false
		...
		compat_otp.By("create a new nodepool")
		replica := 1
		npName := caseID + strings.ToLower(compat_otp.RandStrDefault())
	...
	g.It("Longduration-NonPreRelease-Author:heli-Critical-67278-Critical-69222-[HyperShiftINSTALL] Test embargoed cluster upgrades imperceptibly [Serial]", func() {
		...
		compat_otp.By("check hostedcluster nodes ready")
		installHelper.createHostedClusterKubeconfig(createCluster, hostedCluster)
		o.Eventually(hostedCluster.pollGetHostedClusterReadyNodeCount(""), LongTimeout, LongTimeout/10).Should(o.Equal(2), fmt.Sprintf("not all nodes in hostedcluster %s are in ready state", hostedCluster.name))

		compat_otp.By("ocp-69222 check hosted cluster only expost port 443")
		...
		compat_otp.By("get management cluster cluster version and find the latest CI image")
		hcpRelease := doOcpReq(oc, OcpGet, true, "-n", hostedCluster.namespace, "hc", hostedCluster.name, `-ojsonpath={.spec.release.image}`)
		mgmtVersion, mgmtBuild, err := compat_otp.GetClusterVersion(oc)
		o.Expect(err).ShouldNot(o.HaveOccurred())
		e2e.Logf("hcp image is %s and mgmt cluster image is %s", hcpRelease, mgmtBuild)

		ciImage, err := compat_otp.GetLatestImage(architecture.ClusterArchitecture(oc).String(), "ocp", mgmtVersion+".0-0.ci")
		o.Expect(err).ShouldNot(o.HaveOccurred())

		compat_otp.By("upgrade hcp to latest ci image by controlPlaneRelease")
		...
		compat_otp.By("check clusterversion operator in hcp is updated to ci image")
		o.Eventually(func() bool {
		...
		compat_otp.By("create a new nodepool and check its version is still the old one")
		npName := fmt.Sprintf("np-67278-%s", compat_otp.GetRandomString())
		nodeCount := 1
	...
	g.It("NonPreRelease-Longduration-Author:fxie-Critical-70614-[HyperShiftINSTALL] Test HostedCluster condition type AWSDefaultSecurityGroupDeleted [Serial]", func() {
		...
		var (
			namePrefix          = fmt.Sprintf("70614-%s", strings.ToLower(compat_otp.RandStrDefault()))
		...
		compat_otp.By("Installing the Hypershift Operator")
		defer func() {
		...
		compat_otp.By("Creating a HostedCluster")
		release, err := compat_otp.GetReleaseImage(oc)
		o.Expect(err).ShouldNot(o.HaveOccurred())
		...
		compat_otp.By("Getting default worker SG of the hosted cluster")
		...
		compat_otp.By("Creating a dummy load balancer which has the default worker SG attached")
		...
		compat_otp.By("Delete the HostedCluster without waiting for the finalizers (non-blocking)")
		doOcpReq(oc, OcpDelete, true, "hc", hostedCluster.name, "-n", hostedCluster.namespace, "--wait=false")

		compat_otp.By("Polling until the AWSDefaultSecurityGroupDeleted condition is in false status")
		o.Eventually(func() string {
		...
		compat_otp.By("Start watching the HostedCluster with a timeout")
		...
		compat_otp.By("Now delete the load balancer created above")
		_, err = elbClient.DeleteLoadBalancer(&elb.DeleteLoadBalancerInput{
			LoadBalancerName: aws.String(lbName),
		})
		if err != nil {
		...
		compat_otp.By("Examining MODIFIED events that occurs on the HostedCluster")
	...
	outerForLoop:
		...
		compat_otp.By("Polling until the HostedCluster is gone")
		o.Eventually(func() bool {
	...
	g.It("Longduration-NonPreRelease-Author:heli-Critical-64409-[HyperShiftINSTALL] Ensure ingress controllers are removed before load balancers [Serial]", func() {
		...
		compat_otp.By("create HostedClusters config")
		nodeReplicas := 1
		createCluster := installHelper.createClusterAWSCommonBuilder().
			withName("hypershift-"+caseID).
		...
		compat_otp.By("add watcher to catch the resource deletion info")
		...
		defer func() {
			...
			compat_otp.By("check the ingress controllers are removed before load balancers")
		...
		compat_otp.By("start a goroutine to watch delete time for the hosted cluster svc router-default")
		...
		compat_otp.By("start a goroutine to watch delete time for the hosted cluster ingresscontroller default")
		icName := "default"
	...
	g.It("NonPreRelease-Longduration-Author:fxie-Critical-68221-[HyperShiftINSTALL] Test the scheduler to only accept paired Nodes and check scheduler HCs has two Nodes [Disruptive]", func() {
		...
		var (
			testCaseId         = "68221"
			resourceNamePrefix = fmt.Sprintf("%s-%s", testCaseId, strings.ToLower(compat_otp.RandStrDefault()))
		...
		var (
			...
			deleteServingNode = func(allNodeNames, allMsetNames []string, servingNodeName string) string {
				...
				compat_otp.WaitForNodeToDisappear(oc, servingNodeName, LongTimeout, DefaultTimeout/10)
				return msetName
			}
			...
			waitForHostedClusterDeploymentsReady = func(ns string) {
				compat_otp.WaitForDeploymentsReady(context.Background(), func(ctx context.Context) (*appsv1.DeploymentList, error) {
					return adminKubeClient.AppsV1().Deployments(ns).List(ctx, metav1.ListOptions{})
				}, compat_otp.IsDeploymentReady, LongTimeout, DefaultTimeout/10, false)
			}
		)
		...
		compat_otp.SkipNoCapabilities(oc, "MachineAPI")
		if iaasPlatform != "aws" {
			g.Skip(fmt.Sprintf("Running on %s while the test case is AWS-only, skipping", iaasPlatform))
		}

		compat_otp.By("Getting info about the management cluster")
		...
		compat_otp.By("Creating additional worker nodes through MachineSets on the management cluster")
		...
		for _, msetName := range allMsetNames {
			allWorkerNodeNames = append(allWorkerNodeNames, compat_otp.GetNodeNameByMachineset(oc, msetName))
		}
		...
		compat_otp.By("Creating a MachineHealthCheck for each serving MachineSet")
		...
		compat_otp.By("Adding labels and taints on the serving node pairs and a non serving node")
		...
		compat_otp.By("Creating a hosted cluster with request serving annotation")
		release, err := compat_otp.GetReleaseImage(oc)
		o.Expect(err).ShouldNot(o.HaveOccurred())
		...
		compat_otp.By("Making sure all hosted cluster components are correctly scheduled")
		// No need to check tolerations as the correct scheduling of Pods implies correct toleration settings
		compat_otp.By("Making sure the correct labels and nodeAffinities are set on the request serving components")
		...
		compat_otp.By("Making sure that labels and taints are correctly set on the serving nodes pair")
		...
		compat_otp.By("Making sure the cluster-scheduled annotation is set on the HostedCluster")
		...
		compat_otp.By("Delete the serving node by scaling down the corresponding MachineSet")
		...
		compat_otp.By("Making sure serving components are moved to the other node in the serving node pair")
		...
		compat_otp.By("Delete the new serving node by scaling down the corresponding MachineSet")
		...
		compat_otp.By("Making sure that serving components are moved to a node belonging to the other serving node pair")
		waitForHostedClusterDeploymentsReady(hostedClusterIdentifier)
		...
		compat_otp.By("Making sure that labels and taints are correctly set on the serving node pair")
		checkServingNodePairLabelsAndTaints(hostedClusterIdentifier, servingPairIdx2)

		compat_otp.By("Destroying the hosted cluster")
		installHelper.destroyAWSHostedClusters(createCluster)

		compat_otp.By("Making sure serving nodes are deleted")
		for _, node := range servingPairs[servingPairIdx2] {
			compat_otp.WaitForNodeToDisappear(oc, node, LongTimeout, DefaultTimeout/10)
		}

		compat_otp.By("Making sure two new nodes are created by MAPI")
		...
		compat_otp.By("Making sure that the two new nodes does not contain specific label and taint")
	...
	g.It("Longduration-NonPreRelease-Author:heli-High-64847-[HyperShiftINSTALL] Ensure service type of loadBalancer associated with ingress controller is deleted by ingress-controller role [Serial]", func() {
		...
		var (
			namePrefix  = fmt.Sprintf("64847-%s", strings.ToLower(compat_otp.RandStrDefault()))
...
`, svcName)
		...
		defer func() {
			compat_otp.By("destroy hosted cluster in one goroutine")
			go func() {
				defer g.GinkgoRecover()
				installHelper.destroyAWSHostedClusters(createCluster)
			}()
			...
			if oc.GetGuestKubeconf() != "" {
				compat_otp.By("check LB test SVC is deleted")
				o.Eventually(func() bool {
				...
				compat_otp.By("check HCCO logs that deletion is stuck by LB SVC resources")
				...
				_, err = compat_otp.WaitAndGetSpecificPodLogs(oc, hcpNS, "", hccoPodName, "'Ensuring load balancers are removed'")
				o.Expect(err).NotTo(o.HaveOccurred())

				compat_otp.By("remove ingress-operator debug annotation and scale up ingress-operator")
			...
			compat_otp.By("wait until the hosted cluster is deleted successfully")
		...
		compat_otp.By("annotate the hosted cluster to debug ingress operator")
		...
		compat_otp.By("create LB SVC on the hosted cluster")
...
spec:
  ...
  scaleTargetRef:
    ...
    name: %[1]s`
		...
		compat_otp.By("Inspecting platform")
		compat_otp.SkipNoCapabilities(oc, "MachineAPI")
		...
		compat_otp.By("Creating autoscalers")
		...
		compat_otp.By("Creating extra worker nodes")
		...
		*/
		compat_otp.By("Defining serving pairs")
		e2e.Logf("Getting node name for each machineset")
		var workerNodeNames []string
		msetNames = append(msetNames, extraMsetNames...)
		for _, msetName := range msetNames {
			workerNodeNames = append(workerNodeNames, compat_otp.GetNodeNameByMachineset(oc, msetName))
		}
		...
		compat_otp.By("Adding labels and taints to serving pair 1 nodes and the non serving node")
		...
		defer func() {
			...
			compat_otp.By("Waiting for the serving nodes to be removed before uninstalling the Hypershift Operator")
		...
		compat_otp.By("Creating hosted cluster 1 with request serving annotation")
		release, err := compat_otp.GetReleaseImage(oc)
		o.Expect(err).ShouldNot(o.HaveOccurred())
		createCluster1 := installHelper.createClusterAWSCommonBuilder().
			withName(hc1Name).
		...
		compat_otp.By("Adding labels and taints to serving pair 2 nodes")
		// The osd-fleet-manager.openshift.io/paired-nodes label is not a must for request serving nodes
		defer func() {
		...
		compat_otp.By("Creating hosted cluster 2 with request serving annotation")
		createCluster2 := installHelper.createClusterAWSCommonBuilder().
			withName(hc2Name).
			withNodePoolReplicas(1).
			withAnnotation(hcTopologyAnnotationKey, "dedicated-request-serving-components").
		...
		compat_otp.By("Making sure that non-serving components are scheduled on a default worker node after filling up the non serving node")
...
{{- end }}
    source: {{ .Source }}`))
		...
		compat_otp.By("Checking if there's a need to skip the test case")
		...
		compat_otp.By("Creating an ICSP on the management cluster")
		...
		defer func() {
			...
			compat_otp.By("Restoring the management cluster")
		...
		compat_otp.By("Creating a hosted cluster")
		release, err := compat_otp.GetReleaseImage(oc)
		o.Expect(err).ShouldNot(o.HaveOccurred())
		createCluster := installHelper.createClusterAWSCommonBuilder().
			withName(hcName).
		...
		compat_otp.By("Making sure that OPENSHIFT_IMG_OVERRIDES retains mirroring order from ICSP")
	...
	*/
	g.It("Longduration-NonPreRelease-Author:fxie-Critical-65606-[HyperShiftINSTALL] The cluster can be deleted successfully when hosted zone for private link is missing [Serial]", func() {
		...
		compat_otp.By("Skipping incompatible platforms")
		compat_otp.SkipIfPlatformTypeNot(oc, "aws")
		...
		compat_otp.By("Creating a PublicAndPrivate hosted cluster with external DNS enabled")
		release, err := compat_otp.GetReleaseImage(oc)
		o.Expect(err).ShouldNot(o.HaveOccurred())
		createCluster := installHelper.createClusterAWSCommonBuilder().
			withName(hcName).
		...
		compat_otp.By("Pausing reconciliation")
		defer func() {
			compat_otp.By("Un-pausing reconciliation")
		...
		compat_otp.By("Waiting until the awsprivatelink controller is actually paused")
		// A hack for simplicity
		_, err = compat_otp.WaitAndGetSpecificPodLogs(oc, hostedCluster.getHostedComponentNamespace(), "control-plane-operator", "deploy/control-plane-operator", "awsendpointservice | grep -i 'Reconciliation paused'")
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("Get Route53 hosted zone for privatelink")
		...
		compat_otp.By("Delete Route53 hosted zone for privatelink")
...
spec:
  ...
  nfs:
    ...
    server: 127.0.0.1`, pvName)
		...
		compat_otp.By("Creating a public HostedCluster")
		release, err := compat_otp.GetReleaseImage(oc)
		o.Expect(err).ShouldNot(o.HaveOccurred())
		createCluster := installHelper.createClusterAWSCommonBuilder().
			withName(hcName).
		...
		compat_otp.By("Creating an un-deletable PV in the hosted cluster")
		var pvFile *os.File
		...
		compat_otp.By("Deleting the hosted cluster in a non blocking fashion")
		doOcpReq(oc, OcpDelete, true, "hc", hc.name, "-n", hc.namespace, "--wait=false")

		compat_otp.By("Waiting for the CloudResourcesDestroyed condition to be set")
		o.Eventually(func() bool {
		...
		compat_otp.By("Waiting for the HostedClusterDestroyed condition to be set")
		o.Eventually(func() bool {
		...
		compat_otp.By("Waiting for the HostedCluster to be deleted")
		o.Eventually(func() bool {
	...
	*/
	g.It("Author:fxie-Longduration-NonPreRelease-Critical-73944-[HyperShiftINSTALL] AZURE Etcd Encryption [Serial]", func() {
		...
		compat_otp.By("Creating a keyvault to hold the keys")
		...
		compat_otp.By("Creating keys within the keyvault")
		...
		compat_otp.By("Installing Hypershift Operator")
		installHelper := installHelper{oc: oc, dir: tmpDir, iaasPlatform: iaasPlatform}
		defer installHelper.hyperShiftUninstall()
		installHelper.hyperShiftInstall()

		compat_otp.By("Creating hosted cluster")
		var release string
		release, err = compat_otp.GetReleaseImage(oc)
		...
		compat_otp.By("Specifying a backup key on the HC")
		...
		compat_otp.By("Swapping active & backup key")
		kasResourceVersion = doOcpReq(oc, OcpGet, true, "deploy/kube-apiserver",
			"-n", hc.getHostedComponentNamespace(), "-o=jsonpath={.metadata.resourceVersion}")
		...
		compat_otp.By("Re-encoding all Secrets & ConfigMaps using the current active key")
		hc.encodeSecrets(context.Background())
		hc.encodeConfigmaps(context.Background())

		compat_otp.By("Remove the backup key from HC")
		kasResourceVersion = doOcpReq(oc, OcpGet, true, "deploy/kube-apiserver",
			"-n", hc.getHostedComponentNamespace(), "-o=jsonpath={.metadata.resourceVersion}")
	...
	// Test run duration: about 42 mins
	g.It("Author:lgao-Longduration-NonPreRelease-Critical-83580-[HyperShiftINSTALL] Autoscaler should ignore the topology.k8s.aws/zone-id label for balancing similar nodegroups [Serial] [Disruptive]", func() {
		if iaasPlatform != "aws" {
			g.Skip("IAAS platform is " + iaasPlatform + " while 83580 is for AWS - skipping test ...")
		}
		zones := getAWSMgmtClusterRegionAvailableZones(oc)
		if len(zones) < 2 {
			g.Skip("This testcase needs at least 2 AWS available zones to run, it only has: " + strings.Join(zones, ","))
		}
		// only take the first 2 in case there are more than 2 zones
		availableZones := strings.Join([]string{zones[0], zones[1]}, ",")
		caseID := "83580"
		dir := "/tmp/hypershift" + caseID
		defer os.RemoveAll(dir)
		err := os.MkdirAll(dir, 0755)
		...
		createCluster := installHelper.createClusterAWSCommonBuilder().
			withName("hypershift-" + caseID).
			// this will trigger 2 node pools created, each of which in different zone
			withNodePoolReplicas(1).
			withZones(availableZones).
			// we are testing the image comes from the payload in mgmt cluster
			withReleaseImage(payloadPullSpec)
		...
		compat_otp.By("create HostedClusters node ready")
		installHelper.createHostedClusterKubeconfig(createCluster, hc)
		o.Eventually(hc.pollGetHostedClusterReadyNodeCount(""), LongTimeout, LongTimeout/10).Should(o.Equal(2), fmt.Sprintf("not all nodes in hostedcluster %s are in ready state", hc.name))

		compat_otp.By("The topology.k8s.aws/zone-id label should be in the nodes")
		...
		nodeBalancingIgnoredLabel := "topology.k8s.aws/zone-id"
		nodes := doOcpReq(oc, OcpGet, true, "--kubeconfig", hostedcluster.hostedClustersKubeconfigFile, "nodes", "-l", nodeBalancingIgnoredLabel)
		o.Expect(strings.Count(nodes, "Ready")).Should(o.BeEquivalentTo(2))
		nodePoolStr := doOcpReq(oc, OcpGet, true, "nodepool", "-n", hostedClusterNs, `-ojsonpath={.items[*].metadata.name}`)
		nodePools := strings.Split(nodePoolStr, " ")
		o.Expect(len(nodePools)).Should(o.BeEquivalentTo(2), "expect 2 node pools created")
		npName1 := nodePools[0]
		npName2 := nodePools[1]
		...
		compat_otp.By(fmt.Sprintf("Enable ClusterAutoScaling for hosted cluster %s with Random expander specified and balancingIgnoredLabels set.", hostedcluster.name))
		...
		var (
			autoScalingMax     = 4
			autoScalingMin     = 1
			workloadTemplate   = filepath.Join(compat_otp.FixturePath("testdata", "hypershift"), "workload.yaml")
			parsedWorkloadFile = "ocp-83580-workload-template.config"
		)

		compat_otp.By("Enable autoscaling for the new created NodePools")
		...
		compat_otp.By("Create workload to the hosted cluster to trigger scaling up")
		...
		compat_otp.By(fmt.Sprintf("Check that the nodes in the hosted cluster is auto-scaled to %d", targetNodeCount))
		nodeCount := func() int {
		...
		compat_otp.By("Check that the nodes in 2 node pools should be roughtly even")
		cnt1, err := hostedcluster.getHostedClusterReadyNodeCount(npName1)
		o.Expect(err).NotTo(o.HaveOccurred())
		cnt2, err := hostedcluster.getHostedClusterReadyNodeCount(npName2)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		compat_otp.By("Great, balancing ingore labels works as expected!")
	})
